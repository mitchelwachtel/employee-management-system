...
// These helper functions are used to retreive specific employees, roles, or departments needed for the inquirer prompt or to be displayed in a table.
const {
  selectEmp,
  getEmpArr,
  empQuery,
...
// since this object would be used so much, here is one central place it can be updated or called from
...
// The class Prompt is used to control the user experience
class Prompt {
  ...
  // This start function is called often to ask the user the main prompting question of what they want to do
  start() {
...
// Need the array of departments to be populated before the inquirer prompt tries to use it
    const deptArr = [];
  ...
  async viewEmployeeByManager() {
    ...
    // The options for manager needs to include "No Manager", so the array gets No Manager by default and the other options are pushed to it
    ...
    getEmpArr().then((result) => {
      ...
      inquirer
        ...
        // Here we process all the options. For instance, there could be no employees with a particular manager, so returning the appropriate phrase if no table.
        .then(async (response) => {
  ...
  async viewDepartmentBudget() {
    ...
    getDeptArr().then((result) => {
      ...
      inquirer
        ...
        .then((response) => {
          // Joined the tables to find just the salaries of the employees who exist in just the department chosen AND accumulated those salaries
          getDeptId(response.dept).then(async (dId) => {
            const [rows, fields] = await db.execute(
